...
data_files <- c("data_meta", "data_pm", "data_hourly", "data_daily", "data_diurnal")
file_list <- list()
for(i in data_files){file_list <- append(file_list, exists(i))}
if(FALSE %in% file_list){
  # Looading data; the 'july_api_*' files are imported with the package.
  data_meta <- july_api_meta
  data_pm <- july_api_full
...
# Most basic example
heatmap_cross(data_pm, pm25_atm)

# Further customization. You can avoid washing out the color scale by adding a cap.
heatmap_cross(data_pm, pm25_atm, cap_value = 75, cap_color = "green")
