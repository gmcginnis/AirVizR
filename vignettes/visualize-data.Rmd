...
if (exists("input_startdate") == FALSE) {
  input_startdate <- as.character(min((data_full %>% column_dt("date"))$date))
}
if (exists("input_enddate") == FALSE) {
  input_enddate <- as.character(max((data_full %>% column_dt("date"))$date))
}
...
data_hourly %>% 
  pivot_longer(c("pm25_atm", "pm25_epa_2021"), names_to = "equation", values_to = "pm25") %>% 
  ggplot(aes(x = date_hour, y = pm25, color = equation)) +
  geom_point(alpha = 0.1, size = 1)

data_daily %>% 
  pivot_longer(c("pm25_atm", "pm25_epa_2021"), names_to = "equation", values_to = "pm25") %>% 
  ggplot(aes(x = as.character(date), y = pm25, color = equation)) +
  geom_boxplot()
```
...
Below are examples combining existing PurpleAir and FRM data that exist in the pacage.

```{r combine,eval=FALSE}
library(plyr)

july_meta <- rbind(july_api_meta, july_frm_meta)
tail(july_meta)

july_daily <- rbind.fill(july_api_daily, july_frm_daily)
head(july_daily)
