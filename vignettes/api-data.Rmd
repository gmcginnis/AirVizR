...
data_full <- wrangle_data(raw_data)

# Creating data sets for each grouping option
data_hourly <- apply_functions(data_full, by_day = TRUE, by_hour = TRUE)
data_daily <- apply_functions(data_full, by_day = TRUE, by_hour = FALSE)
data_diurnal <- apply_functions(data_full, by_day = FALSE, by_hour = TRUE)

# Apply date and/or hour tags to the original data set, if specified in the inputs
if (run_date_grouping == TRUE){
  data_full <- apply_date_tags(data_full)
}
if (run_hour_grouping == TRUE) {
  data_full <- apply_hour_tags(data_full)
}
```
