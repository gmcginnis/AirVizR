...
# See ambient_temperature() for more information
...
If more than 100 IDs are detected, a message will appear in the console to warm of a large data load. This is also why `get_ids()` is not automatically integrated into `get_area_pat()`.
...
remove(input_stateCode, input_north, input_east, input_south, input_west, input_labels, ids, pas_area)
```
...
data_hourly <- apply_functions(data_pm, by_day = TRUE, by_hour = TRUE)
data_daily <- apply_functions(data_pm, by_day = TRUE, by_hour = FALSE)
data_diurnal <- apply_functions(data_pm, by_day = FALSE, by_hour = TRUE)

# Apply date and/or hour tags to the original data set, if specified in the inputs
if (run_date_grouping == TRUE){
  data_pm <- data_pm %>% apply_date_tags()
}
if (run_hour_grouping == TRUE) {
  data_pm <- data_pm %>% apply_hour_tags()
}
```
...
# OPTIONAL
# Cleaning the global environment
remove(temperature_change, input_drop_hi, results, run_date_grouping, run_hour_grouping, include_outside, include_inside)
remove(list = c(ls(pattern = "input_(hour|date)_")))
# It is recommended to keep raw_meta and raw_data
```
