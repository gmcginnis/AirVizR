...
#' @seealso \code{\link[AirSensor]{pas_load()}}
...
get_area_pas <- function(state_code = input_stateCode,
  ...
  AirSensor::setArchiveBaseUrl(archive_url)
  ...
  if (is.null(state_code) == TRUE) {
    ...
    pas_state <- AirSensor::pas_filter(pas, stateCode == state_code)
    print(paste("Sensors now filtered for the state of", state_code))
  }
  
  pas_area <- AirSensor::pas_filter(
    AirSensor::pas_filterArea(
      pas = pas_state,
      w = west,
      e = east,
      s = south,
