...
settings_shapes <- function(dataset = dataset, meta = data_meta) {
  ...
  if ("location" %in% colnames(dataset) == FALSE) {
    dataset <- dplyr::left_join(dataset, meta)
  }
  ...
  shape_set <- ggplot2::scale_shape_manual(values = stats::setNames(shapes_df$shape, as.character(shapes_df$location)))
  shape_guide <- ggplot2::guides(shape = ggplot2::guide_legend(override.aes = list(fill = "black", color = "black"), nrow = shape_nrow))
